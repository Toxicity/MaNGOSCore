...
void WorldSession::HandleCharCreateOpcode( WorldPacket & recv_data )
{
    ...
    CharacterDatabase.PExecute("INSERT INTO characters_begin (guid) VALUES (%u)" , pNewChar->GetGUIDLow());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder *holder)
{
    ...
    QueryResult *result = CharacterDatabase.PQuery("SELECT begin FROM characters_begin WHERE guid='%u'", pCurrChar->GetGUIDLow());
    Field *fields = result->Fetch();
    ...
    if(result)
    {
        if(begin == 0)
        {    
          pCurrChar->CastSpell(pCurrChar, 38505, true);
        }
        else
        {
          pCurrChar->RemoveAurasDueToSpell(38505);
        }
    }

    QueryResult *result2 = CharacterDatabase.PQuery("SELECT guildid FROM guild");
    if(result2)
    {
       if (pCurrChar->GetTeam() == ALLIANCE )
	   {
        uint32 Guild_Alli = sWorld.getConfig(CONFIG_UINT32_START_GUILD_ALLI);
        uint32 defaultplayerguildid = Guild_Alli;
      ...
      else
      {
        ...
        uint32 Guild_Horde = sWorld.getConfig(CONFIG_UINT32_START_GUILD_HORDE);
        uint32 defaultplayerguildid = Guild_Horde;
        Guild *playerguild = sGuildMgr.GetGuildById(defaultplayerguildid);
        if (defaultplayerguildid == 0)
        {
          sLog.outError("Player default GuildID can't be 0.");
